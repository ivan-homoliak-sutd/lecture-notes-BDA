...
This section serves as an  introduction to the course of
...
systems. These include a brief examination of cryptographic hash
...
\begin{itemize}
	\tightlist
	\item
	Gain theoretical and practical knowledge in the field of blockchains and decentralized applications, 
	types of blockchains, their consensual protocols, and pros/cons.
	\item
	Understand the terminology, unique properties of blockchains,
...
The following list contains current and former course instructors who helped in running this course.
\begin{itemize}
	\item
	\textbf{Ivan Homoliak} (Course Guarantor). Ivan is
	...
	Decentralized Applications in Blockchains,''~\cite{homoliak2025towards} focuses on several research topics including standardization of threat modeling~\cite{homoliak2020security},  electronic voting in blockchains~\cite{homoliak2023bbb,venugopalan2023always,stanvcikova2022sbvote,homoliak2025votemate}, the security
	and scalability of consensus protocols~\cite{perevsini2023incentive,perevsini2021dag,perevsini2023dag,budinsky2023fee,homoliak2025pos}, the intersection of Trusted
	Execution Environments (TEE) with blockchain technology~\cite{homoliak2020aquareum}, Central Bank
	Digital Currencies (CBDC)~\cite{homoliak2023cbdc}, authentication/cryptocurrency wallets~\cite{homoliak2024sok,homoliak2020smartotps,perevsini2025featherwallet,drga2023detecting} and mixers~\cite{mariani2025sok}.
	In addition, Ivan focuses on other topics in consensus protocols such as DoS on the leader~\cite{burianova2025secret} in PoS consensus protocols, Proof-of-Useful-Work with mining SNARKs/zk-SNARKs~\cite{oleksak2025zk}, and Proof-of-Social-Capital~\cite{mariani2025proof}.
	
	\item
	\textbf{Vladimír Veselý} (Course Coordinator). Vladimír's
	...
	activities on the blockchain. His related research has involved correlating
	dark market activities with cryptocurrency transactions~\cite{dolejvska2022shedding,dolejvska2023busting}, blockchain forensics~\cite{vesely2019detect}, and exploring
	the use of blockchains for storing personal named records~\cite{koutensky2025using}.
	
	\item
	\textbf{Martin Perešíni} (Teaching Assistant). Martin is a Ph.D.~student (under supervision of Dr. Homoliak) whose research is
	...
	investigates incentive schemes and threats such as selfish mining~\cite{peresini2026selfish}.
	
	\item
	\textbf{Samuel Olekšák} (Teaching Assistant). Samuel is a Ph.D.~student (under supervision of Dr. Homoliak) specializing in the
	...
	Proof-of-Useful-Work for mining zero-knowledge proofs~\cite{oleksak2025zk}, aiming to make the mining process more computationally meaningful.
	
	\item
	\textbf{Ivana Stančíková} (Teaching Assistant). Ivana is A former Ph.D. student (under supervision of Dr. Homoliak) whose focus revolved around blockchain-based e-voting protocols and the problem of their scalability~\cite{venugopalan2023always,stanvcikova2022sbvote}.
\end{itemize}

\subsubsection{Motivation and Expected
...
rigorous education in the technical aspects of BDA.
...
\begin{itemize}
	...
	disadvantages.
	\item
	\textbf{Technical Terminology and Concepts}: A knowledge
...
%	their own blockchain-based solutions.
	The lecture series covers a range of topics, starting with the basic principles of cryptography and distributed systems. The curriculum then moves on to examine existing blockchain platforms, their unique features, and the challenges they face. A large portion of the lectures focuses on security and privacy issues, which are essential for designing strong decentralized systems. The course ends with practical training on developing decentralized applications (DAPPs) and smart contracts, giving students the skills they need to create and deploy their own blockchain solutions.
...
%	two distinct tracks:
	A key part of the course is a semester-long project, which is worth 40 points of the final grade. Students need to earn at least one point to pass. The project has two options:
	
	\begin{itemize}
		\tightlist
		\item
		\textbf{Mainstream Track}: This choice involves a standardized task, such as extending the functionality of an ERC20 token. This track aims to give students a solid foundation in smart contract development.
		\item
		\textbf{Individual Track}: This option lets students work on a project that matches their specific interests, which can relate to their bachelor's or master's thesis. This allows for in-depth research and development in a focused area of blockchain technology.
	\end{itemize}
	\item
...
%	required to pass the course. There will be no mid-term exam.
	A final exam will take place at the end of the semester, covering all the topics discussed in the lectures. The exam is worth 60 points, and students must score at least 30 to pass the course. There will be no mid-term exam.
\end{itemize}
\fi
...
The security of a system, especially a decentralized one like blockchain, relies on a clear and detailed \textbf{attacker model}. This model outlines the capabilities and resources of a potential attacker. By establishing a strong attacker model, we can design and assess systems with a clear understanding of the threats they are meant to counter. In blockchain security, the attacker model usually includes the following assumptions:
...
The attacker is assumed to have control over the network, allowing actions such as eavesdropping on communication and performing man-in-the-middle (MITM) attacks. This assumption reflects the fact that the communication infrastructure is not inherently secure.
...
%	any realistic amount of computational power.
	It is assumed that the cryptographic primitives used in the blockchain, like hash functions and digital signature schemes, are secure. This means the attacker cannot break these primitives with any realistic computational power.
	\item
	\textbf{Authentication Factors}: 
%	In systems that employ multi-factor
...
%	unable to steal both factors.
	In systems that use multi-factor authentication, it is often assumed that an attacker cannot compromise more than a certain number of authentication factors. For example, in a two-factor authentication scheme, the attacker is assumed to be unable to steal both factors.
	\item
	\textbf{Consensus Power}: 
%	A critical assumption in permissionless
...
%	staked capital or the number of equal consensus nodes, respectively.
	A key assumption in permissionless blockchains is that an attacker cannot control more than a specific threshold of the network's consensus power. In Proof-of-Work systems like Bitcoin, this threshold is usually 51\% of the total hash rate. In Proof-of-Stake and Proof-of-Authority systems, it is often 33\% of the total staked capital or the number of equal consensus nodes, respectively.
\end{itemize}
...
It is essential to understand that while cryptography is important, it is not the solution to all security problems. The practical security of a blockchain system also depends on several other factors:

\begin{itemize}
	\tightlist
	\item
	\textbf{Implementation Issues}: 
%	Flaws in the implementation of
...
%	implementation is a common source of security breaches.
	Flaws in implementing cryptographic protocols can create vulnerabilities, even if the underlying cryptographic primitives are secure. A gap between the mathematical definition of a protocol and its software implementation is a common cause of security issues.
	\item
	\textbf{Side-Channel Attacks}: 
%	These attacks exploit information
...
%	device to infer secret keys.
	These attacks take advantage of information leaked from the physical implementation of a system, rather than weaknesses in the algorithms. For example, an attacker might examine the power use or electromagnetic radiation of a device to discover secret keys.
	\item
	\textbf{Key Management}: 
%	The secure generation, storage, and
...
%	system, regardless of the strength of the cryptographic algorithms.
	Proper generation, storage, and management of cryptographic keys are crucial. A compromised private key can threaten the security of the entire system, no matter how strong the cryptographic algorithms are.
...
%	system.
	There is frequently a trade-off between security and performance. For instance, while zero-knowledge proofs can provide strong privacy guarantees, their computational demands can be high, affecting the scalability and efficiency of the system.
\end{itemize}
...
\begin{itemize}
	...
	\item
	\textbf{Collision Resistance}: Collision resistance is typically considered stronger than pre-image and second pre-image resistance. It asserts that it is
...
\subsubsection{Lamport's Hash Chains}\label{lamports-hash-chains}
...
\begin{figure}[t]
	%	\vspace{-0.3cm}
	\begin{center}
		\includegraphics[width=0.7\textwidth]{./figs/hash-chain-with-data.pdf} 

		\caption{Hash chain with data~\cite{hash-link}.}		
...
\begin{figure}[b]
	%	\vspace{-0.3cm}
	\begin{center}
		\includegraphics[width=0.6\textwidth]{./figs/merkle-tree.pdf} 
...
\begin{figure}[t]
	%	\vspace{-0.3cm}
	\begin{center}
		\includegraphics[width=0.9\textwidth]{./figs/combination-mk-hc.jpg} 
...
\begin{itemize}
	...
	\textbf{hiding} property is also satisfied (assuming the high enough entropy of \texttt{x} that is not guessable).
	\item
...
\begin{itemize}
	...
	\item
	\textbf{Verification}: 
%	To verify a signature, a recipient uses the
...
%	valid (see \autoref{fig:signatures}.
	To verify a signature, the
	recipient uses the sender's public key, the message, and the signature.
	The verification algorithm returns a boolean value indicating whether
	the signature is valid (see \autoref{fig:signatures}).
\end{itemize}
...
Digital signatures offer several important security features:
...
%	signature that can be verified with the public key.
	A signature that is valid shows that the
	message was actually signed by the holder of the corresponding private
	key. It is because the owner of the private key would be the only
	person able to create a valid signature that can be verified by the
	public key.
	
	\item
	\textbf{Integrity}: 
%	The signature ensures that the message has not
%	been altered since it was signed. Any modification to the message
%	would result in an invalid signature.
	The signature guarantees that the message has not been
	modified after it was signed. If any changes are made to the message,
	the signature will become invalid.
	\item
	\textbf{Non-Repudiation}: 
%	The signer cannot plausibly deny having
...
%	compromised.
	The signer cannot reasonably deny that they signed
	the message, as only they have the private key. It is worth
	mentioning, however, that this property may be challenged if the
	signer can legitimately claim that their private key was compromised.
...
%	discrete logarithm problem or the integer factorization problem.
	Without access to the private key, it would be
	computationally impossible for an attacker to produce a valid
	signature for a new message. This property depends on the
	computational hardness of the mathematical problems, e.g., the discrete
	logarithm problem or the integer factorization problem that
	underpin the cryptographic primitives.
\end{itemize}
...
Within the domain of blockchains, digital signatures serve the purpose
of transaction authorization. A user signing a transaction with their
private key is what a cryptocurrency transfer or smart contract
interaction requires. They, by their signature, prove their
ownership of the funds or assets and that they have approved the
transaction.
...
A key impetus behind blockchain technology was the fixed
disadvantages of the existing traditional centralized information
systems. These systems, despite the fact that they are widely used,
have a number of vulnerabilities that can compromise their security,
...
%	can render the entire system inoperable.
	...
	failure of one component. If a central server's hardware malfunctions, a
	software bug is introduced, or the server is targeted successfully during
	a denial-of-service (DDoS) attack, the entire system will become
	nonfunctional.
	...
	\item
	\textbf{Lack of Data Integrity}: A sinister insider with access privileges may
	compromise the data in a centralized database by making changes and
	deletions that will be untraceable. Consequently, it is a challenge to
...
%	being handled correctly.
	Users are usually kept in the dark about the
	internal processes of centralized systems. There is no way for
	outsiders to verify objectively whether the system is being used in
	the intended manner or whether the data is being managed properly.
\end{itemize}

Blockchain technology offers a paradigm shift, providing a decentralized
alternative that addresses these challenges by its inherent
properties:
...
%	system can continue to operate even if some nodes go offline.
	Distribution of the ledger over a network of
	nodes allows blockchains to get rid of a single point of failure. The
	network will remain functional even if some nodes are disconnected.
	\item
...
%	and accessible to all.
	All valid transactions are eventually included
	in a permissionless blockchain. There is no individual who can forbid
	the transactions, thereby ensuring that the network stays free and
...
%	tamper-evident record of all activity on the network.
	Since a blockchain's structure is inherently
	append-only, cryptographic hashes provide the necessary security to
	prevent or at least drastically limit the ability to change or remove
	old transactions. Hence, the network has a permanent and tamper-evident
...
%	valid and that the rules of the protocol have been followed.
	Anyone can unravel and verify the full blockchain's
	history. Thus the system can be independently audited to confirm the
	validity of all transactions and the correct implementation of the
	protocol.
	\item
...
%	accountability, as all actions are open to scrutiny.
	All transactions taking place on public blockchains
	can be seen by every participant. This open nature of the network
	characterized by full disclosure of information strengthens trust and
	accountability, given that anyone's actions may be publicly reviewed
\end{itemize}

\subsubsection{High-Level View of Blockchains}\label{high-level-view-of-blockchains}
...
\begin{figure}[t]
	...
	\begin{center}
		...
		\caption{Participants in blockchains~\cite{homoliak2019security}.}		
...
(P2P) network overlaid on the underlying network using a gossiping protocol (see \autoref{fig:network-overlay}). Messages are sent only to a
...
The network propagation delay is a crucial factor. 
A 2013 study on
Bitcoin showed that the mean time for a block to be seen by 95\% of the
nodes was 12.6 seconds, with a maximum of 40 seconds~\cite{decker2013information}.

\begin{figure}[t]
	%	\vspace{-0.3cm}
	\begin{center}
		\includegraphics[width=0.6\textwidth]{./figs/network-overlay.png} 
		\caption{Overview of network in blockchains~\cite{homoliak2020security}.}		
