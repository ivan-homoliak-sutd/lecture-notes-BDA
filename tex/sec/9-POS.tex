This section provides a comprehensive examination of Proof-of-Stake
(PoS) consensus mechanisms, which have emerged as a significant
...
Ouroboros, and Ethereum's Casper and Gasper}. Finally, we will explore
the related challenge of blockchain \textbf{oracles} and how systems can securely
access external data, covering solutions like \textbf{TownCrier, TLS-N, and
PDFS}. Through this exploration, readers will gain a nuanced
understanding of the theoretical underpinnings and practical
implementations of Proof-of-Stake and its surrounding ecosystem.
...
\subsection{Introduction to
	Proof-of-Stake}\label{section-1-introduction-to-proof-of-stake}

\subsubsection{The Motivation for
	Proof-of-Stake}\label{the-motivation-for-proof-of-stake}
...
of Proof-of-Work (PoW) blockchains.
%
...
\begin{figure}[t]
	%	\vspace{-0.3cm}
	\begin{center}
%		\includegraphics[width=0.9\textwidth]{./figs/btc-energy-consumption.png}
	\ih{todo}
...
\begin{itemize}
	...
	\item
	\textbf{Higher Throughput}: Faster block creation times can lead to
...
\subsubsection{2.3: Denial of Service (DoS) on a
...
\begin{itemize}
	\tightlist
	\item
	\textbf{Private Leader Election (via VRF)}: As pioneered by Algorand,
...
Function (VRF). A leader is selected privately and proposes a new block.
A committee of \textasciitilde1000 nodes, also selected by VRF, then
votes on the proposed block to ensure Byzantine tolerance.

\subsubsection{Ouroboros}\label{ouroboros}

Ouroboros is a family of provably secure PoS protocols developed for the
Cardano blockchain.
...
\begin{itemize}
	\tightlist
	\item
	\textbf{Casper the Friendly Finality Gadget (FFG)}: Casper FFG is not
	...
	\begin{itemize}
		...
		blocks are considered permanent.
		...
		\emph{A diagram illustrating the finalization process in Casper
			FFG.}
		\item
	...
	\emph{The GHOST rule combined with the Casper finality gadget.}
\end{itemize}
...
A blockchain is an isolated environment. The ``oracle problem'' is the
challenge of providing smart contracts with reliable data from the
external world.

\subsubsection{TownCrier (TC)}\label{towncrier-tc}

TownCrier acts as a trusted proxy between a blockchain and HTTPS
websites, using a Trusted Execution Environment (TEE) like Intel SGX. *
\textbf{Pros}: Easy integration, no changes needed for website
operators. * \textbf{Cons}: Trusts the TEE manufacturer (a single point
of failure), and several attacks have been demonstrated against Intel
SGX.

\subsubsection{TLS-N}\label{tls-n}

TLS-N adds a non-repudiation layer to TLS, allowing for the generation
of privacy-preserving, non-interactive proofs of the contents of a TLS
session. * \textbf{Pros}: More general and powerful, with extra features
like privacy. * \textbf{Cons}: Requires changes to the TLS specification
...
\subsubsection{Provable Data Feeds
	(PDFS)}\label{provable-data-feeds-pdfs}

In this model, data providers maintain an append-only database (like a
Merkle tree) and sign their contract locations via TLS. The contract API
allows for updating the database root and proving the authenticity of
data via Merkle proofs. * \textbf{Pros}: Direct TLS authentication, easy
integration. * \textbf{Cons}: Website operators have to deploy it.
...
This section has provided a detailed exploration of Proof-of-Stake
