...
transactions and blocks, and the use of Merkle-Patricia Tries to
...
\begin{itemize}
	...
	\item
	Understand the purpose and function of the Merkle-Patricia Trie in
...
\begin{enumerate}
	\tightlist
	\item Specify \texttt{startGas} and \texttt{gasPrice}
	\item  Assert(\texttt{startGas}*\texttt{gasPrice} $\leq$ \texttt{balanceOfCaller})
	\item  \texttt{balanceOfCaller} -= \texttt{startGas}*\texttt{gasPrice}
	\item \texttt{tmpGas} = \texttt{startGas} ; \texttt{gasRefund} = 0
	\item Execute code, deducting from \texttt{tmpGas}
	\begin{itemize}
		\item \texttt{deducted} = ExecutionCost(tx)
		\item \textbf{If} \texttt{deducted} $<$ 0 \textbf{then} \\
		\hspace{0.5cm} ~~~~\texttt{gasRefund} += $|$\texttt{deducted}$|$ \\
		\textbf{else} \\
		\hspace{0.5cm} ~~~~\texttt{tmpGas} -= \texttt{deducted} \\
		\item Assert(\texttt{tmpGas}$\geq$ 0)
	\end{itemize}
	
	\item Refund \texttt{gasRefund} + \texttt{tmpGas} to \texttt{balanceOfCaller}	
\end{enumerate}



\subsubsection{EIP-1559}\label{gas-and-the-ethereum-virtual-machine-evm}
The EIP-1559~\cite{eip1559} was included in London's hardfork that occurred on 5th of August 2021, and its main contribution is to optimize the fee market system.
In particular, this EIP introduces 3 elements of gas:
\begin{itemize}
	\item \textbf{Base Fee} -- price per unit of gas, which is burnt. It is set by the network. Blocks can be extended to 200\%.
	...
	\item \textbf{Fee Cap} -- represents the maximum amount of gas a user is willing to pay.	
\end{itemize}
This EIP improves economics of Ethereum since base we must always be ``paid'' in ETH, while before it could be paid in any currency or token of the exchange with the full node that handled the fee internally.
...
\begin{figure}[t]
	%	\vspace{-0.3cm}
	\begin{center}
		\includegraphics[width=0.7\textwidth]{./figs/eth-block.jpg}
...
\begin{itemize}
	...
	hashes of three Merkle-Patricia Tries that store the global state, the
...
\subsubsection{The Global State and Merkle-Patricia
	Tries}\label{the-global-state-and-merkle-patricia-trees}
...
\begin{figure}[t]
	%	\vspace{-0.3cm}
	\begin{center}
		\includegraphics[width=0.7\textwidth]{./figs/eth-state-change.jpg}
...
sophisticated data structure called a \textbf{Merkle-Patricia Trie
...
\end{compactenum}
The example of the state trie built using MPT is depicted in \autoref{fig:eth-mpt}, and it contains 4 accounts.
...
\begin{compactenum}
	\item \textbf{Add Element}: we hash a new element with k hash functions and set 1 on obtained positions.
	\item \textbf{Query Element}: we hash a queried element with k hash functions and if 1 is on all positions, return true. Otherwise, return false (see \autoref{fig:eth-bloom-filter}).
\end{compactenum}
Bloom filter is often used as a prefilter for expensive storage-lookup queries (see \autoref{fig:bloom-app}).
%
\begin{figure}[t]
	%	\vspace{-0.3cm}
	\begin{center}
		\includegraphics[width=0.9\textwidth]{./figs/bloom.png}
		\caption{An example of the Bloom filter with 3 hashing functions  and 3 elements. Element $w$ in the query is not included in the Bloom filter.}		
...
This section has provided a brief introduction to Ethereum, the
...
\begin{itemize}
	...
	\item
	\textbf{Merkle-Patricia Tries}: The sophisticated data structure used
...
\begin{itemize}
	...
	\item
	\textbf{Merkle-Patricia Trie (MPT)}: A hybrid data structure that
